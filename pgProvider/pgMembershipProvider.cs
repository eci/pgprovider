...
namespace pgProvider
{
    public class pgMembershipProvider : MembershipProvider
    {
        ...
        public pgMembershipProvider(ILog logger)
        {
            _logger = logger;
        }

        public pgMembershipProvider()
            : this(LogManager.GetLogger<pgMembershipProvider>())
        {
        ...
        {
            _logger.Debug(d => d("ChangePassword(string, string, string) called for '{0}'", username));
            if (ValidateUser(username, oldPassword))
            {
                try
                {
                    ...
                    _logger.Info(i => i("Password changed for '{0}'", username));
                    return true;
                }
                catch (Exception ex)
                {
                    _logger.Error("Error attempting to change password.", ex);
                }
            }
        ...
        {
            _logger.Debug(d => d("ChangePassword(string, string) called for '{0}'", username));
            ...
            switch (EncryptionMethod)
            {
                case EncryptionMethods.Hash:
                    _logger.Debug(d => d("Hashing new password..."));
            ...
            using (var conn = new NpgsqlConnection(ConnectionString))
            {
                _logger.Debug(d => d("Updating the password/ salt data for '{0}' in the database...", username));
                conn.Open();
            ...
            _logger.Debug(d => d("Unlocking the account for '{0}' if it was locked...", username));
            UnlockUser(username);

            _logger.Debug(d => d("ChangePassword(string, string) for '{0}' complete.", username));
        }
            ...
            string newPasswordQuestion, string newPasswordAnswer)
        {
            ...
            if (ValidateUser(username, password))
            {
                try
                {
                    ...
                    _logger.Info(i => i("Password Question and answer have been updated for '{0}'", username));
                    return true;
                }
                catch (Exception ex)
                {
                    _logger.Error("Error attempting to change the user's question and answer.", ex);
                }
                _logger.Info(i => i("Password validation failed for user '{0}'", username));
            }
            return false;
        }
            ...
            string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status)
        {
            ...
            {
                _logger.Error("General Error creating the user record.", ex);
                throw ex;
            }

            try
            {
                _logger.Debug(d => d("Setting the password..."));
                ValidatePasswordComplexity(password);
                ChangePassword(username, password);
            }
            catch (PasswordTooShortException ex)
            {
                _logger.Error("Supplied password is too short.", ex);
            ...
            {
                _logger.Error("Password does not meet complexity requirements.", ex);
                status = MembershipCreateStatus.InvalidPassword;
                DeleteUser(username, true);
                return null;
            }
            ...
            if (_RequiresQuestionAndAnswer)
                ...
                {
                    _logger.Error("Invalid Question.", ex);
                ...
                {
                    _logger.Error("Invalid Answer.", ex);
                ...
                {
                    _logger.Error("General Error.", ex);
                    DeleteUser(username, true);
                    throw ex;
                }
            }
            ...
            _logger.Info(i => i("Created user '{0}'.", username));
            return GetUser(username, false);
        }

        protected void UpdateQAndA(string username, string question, string answer)
        {
            _logger.Debug(d => d("UpdateQAndA(string, string, string) for user '{0}' beginning...", username));
            ...
            catch (Exception ex)
            {
                if (_RequiresQuestionAndAnswer)
                {
                    _logger.Error("Questions and Answers are required to be provided; but they are not properly supplied.",
                        ex);
                    throw ex;
                }
            ...
            _logger.Debug(d => d("Updating the Q&A for '{0}' in the database...", username));
            using (var conn = new NpgsqlConnection(ConnectionString))
            {
                conn.Open();
            ...
            _logger.Debug(d => d("Successfully updated the database with question and answer info."));
        }
        ...
        {
            _logger.Debug(d => d("CreateUser('{0}', '{1}', '{2}') beginning...", username, email, isApproved));
        ...
        public override bool DeleteUser(string username, bool deleteAllRelatedData)
        {
            ...
            {
                _logger.Error(string.Format("Failed to delete user '{0}'.", username), ex);
                return false;
            }
        }
        ...
        {
            _logger.Debug(d => d("GetAllUsers({0}, {1}, out int)", pageIndex, pageSize));
            if (pageIndex < 0 || pageSize < 1)
            {
                totalRecords = 0;
                return new MembershipUserCollection();
            ...
            using (var conn = new NpgsqlConnection(ConnectionString))
            {
                ...
                using (var comm = new NpgsqlCommand("get_all_users", conn))
                {
                    ...
                    using (var reader = comm.ExecuteReader())
                    {
                        ...
                        if (users.Count == 0)
                        {
                            _logger.Debug(d => d("No records collected."));
                            totalRecords = 0;
                            return new MembershipUserCollection();

                        }
                        _logger.Debug(d => d("{0} record(s) collected.", users.Count));
                        return PaginateUserCollection(users, pageIndex, pageSize, out totalRecords);
                    }
                }
            }
        ...
        {
            _logger.Debug(d => d("GetPassword: username={0}, answer={1}", username, answer));
            ...
            if (ValidateAnswer(username, answer))
            {
                ...
                _logger.Debug(d => d("GetPassword: username={0}, answer={1}, password={2}", username, answer, password));
                return password;
            }
            _logger.Warn(w => w("Password fetch failed for user '{0}'; the answer was incorrect.", username));
            return null;
        }
        ...
        public override string ResetPassword(string username, string answer)
        {
            ...
            {
                _logger.WarnFormat("User '{0}' failed to validate the security question.", username);
                return null;
            }
        ...
        public override bool UnlockUser(string userName)
        {
            try
            {
                _logger.Debug(d => d("Unlocking user '{0}'...", userName));
                if (string.IsNullOrEmpty(userName)) throw new ArgumentException("username parameter is required.");
                ...
                using (var conn = new NpgsqlConnection(ConnectionString))
                {
                    ...
                    using (var comm = new NpgsqlCommand("unlock_user", conn))
                    {
                        ...
                        comm.Parameters.Add("_application_name", NpgsqlTypes.NpgsqlDbType.Varchar, 250).Value =
                            _ApplicationName;
                        comm.ExecuteNonQuery();
                        _logger.Info(i => i("User '{0}' has been unlocked.", userName));
                        return true;
                    }
                }
            }
            ...
            {
                _logger.Error("Failed to unlock user.", ex);
                return false;
            }
        }
        ...
        public override void UpdateUser(MembershipUser user)
        {
            try
            {
                _logger.Debug(d => d("Persisting changes to user {0} ({1})...", user.ProviderUserKey, user.UserName));
            ...
            {
                _logger.Error("Failed to update user.", ex);
            }
        }
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            {
                _logger.Warn(w => w("Password null or empty for use '{0}'.", username));
                return false;
            }
            ...
            if (creds == null)
            {
                _logger.Warn(w => w("Credentials were not available for user '{0}'.", username));
                return false;
            }

            if (LockoutInEffect(creds.LockedOutAsOf))
            {
                _logger.Warn(w => w("User '{0}' is locked out and cannot authenticate.", username));
                return false;
            }
            ...
            switch (EncryptionMethod)
            {
                ...
                case EncryptionMethods.ReversibleSymmetric:
                    ...
                    break;
                default:
                    _logger.Error("The encryption method was not properly set.  User validation is not possible.");
                    return false;
            }
            ...
            {
                _logger.Info(i => i("User '{0}' successfully authenticated.", username));
                return true;
            }
            if (validated && !user.IsApproved)
            {
                _logger.Warn(w => w("User '{0}' successfully authenticated, but the account is not approved.", username));
            }
            else
            {
                _logger.Warn(w => w("User '{0}' failed to authenticate.", username));
            }
        ...
        protected void RecordLoginEvent(string username, bool success)
        {
            try
            {
                _logger.Debug(d => d("Recording Login Event: {0}, success: {1}...", username, success));
                if (string.IsNullOrEmpty(username)) throw new ArgumentException("Username parameter is required.");
